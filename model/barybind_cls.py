...
def save_features_per_sample(feat_v,feat_a,feat_t,feat_b, batch, output_dir):
    """
    每个样本保存为一个单独的 .mat 文件
    """
    feat_v = feat_v.cpu().detach()           # (B, D1)
    feat_a = feat_a.cpu().detach()           # (B, D2)
    feat_t = feat_t.cpu().detach()           # (B, D3)
    feat_b = feat_b.cpu().detach()  # (B,D4)
    
    B = feat_v.size(0)  # batch size
    os.makedirs(output_dir, exist_ok=True)  # 创建输出目录

    for i in range(B):
        sample_id = batch.get('ids', [f'index_{i}'])[i]  # 使用 ids 或者视频 ID 作为文件名
        save_path = os.path.join(output_dir, f'{sample_id}.mat')  # 每个样本保存为单独的 .mat 文件

        # 保存每个样本的特征到 .mat 文件
        sio.savemat(save_path, {
            'feat_v': feat_v[i].numpy().flatten(),
            'feat_a': feat_a[i].numpy().flatten(),
            'feat_t': feat_t[i].numpy().flatten(),
            'feat_b': feat_b[i].numpy().flatten()
        })
...
class BaryBind(MMGeneralModule):
    ...
    def forward_ret(self, batch, task, compute_loss=True):
        ...
        # # 计算 feat_v, feat_a, feat_t, caption_output_pooled 等特征
        ...
        # 保存每个样本的特征到指定路径
        output_dir = "./datasets/srcdata/vgg/feature/sea"  # 修改为实际路径
        save_features_per_sample(feat_v,feat_a,feat_t,feat_b, batch, output_dir)
    ...
    def forward_qa(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            evaluation_dict = {} 
            num_question = [len(qs) for qs in raw_questions] ### 
    ...
    def forward_ret_vast27m(self, batch, task):
        ...
        if compute_loss:
            ...
            for task in subtasks:
                ...
                assert task in ['tv','ta','tva','tvs','tvas']
                if task == 'tv':
                    feat_b = self.batch_get(batch,'feat_b_vision_caption')
                    caption_tokens = self.batch_get(batch, 'vision_caption_tokens')
                elif task == 'ta':
                    feat_b = self.batch_get(batch,'feat_b_audio_caption')
                    caption_tokens = self.batch_get(batch, 'audio_caption_tokens')
                else:
                    feat_b = self.batch_get(batch,'feat_b_omni_caption')
