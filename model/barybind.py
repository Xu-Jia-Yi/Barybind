...
class BaryBind(MMGeneralModule):
    """ VLP pretraining """
    def __init__(self, config):
        ...
        self.potentials = Potentials(
            num_potentials = self.config.modality_num, channels = self.config.contra_dim)
        self.Lambda = [1 / self.config.modality_num for i in range(self.config.modality_num)]
    ...
    def batch_get(self, batch, key):
        if key in batch:
            ...
            caption_output = self.batch_get(batch, 'caption_output')
            caption_output_pooled = self.pool_text_for_contra(caption_output)
            feat_b = self.contra_head_t(caption_output_pooled)
    ...
    def forward_ret(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            feat_b = self.batch_get(batch,'feat_b')
            feat_b_all = concat_all_gather(feat_b)
            ...
            if "raw_subtitles" in batch.keys():
                if "depth_pixels" in batch.keys():
                    volume = volume_computation5(feat_b,feat_v_all,feat_a_all,feat_s_all,feat_d_all)
                    volume = volume / self.contra_temp
                else:
                    volume = volume_computation4(feat_b,feat_v_all,feat_a_all,feat_s_all)
                    volume = volume / self.contra_temp
            else:
                volume = volume_computation3(feat_b,feat_v_all,feat_a_all)
                volume = volume / self.contra_temp
            #AreaT (Video,batch_all)
            if "raw_subtitles" in batch.keys():
                if "depth_pixels" in batch.keys():
                    volumeT = volume_computation5(feat_b_all,feat_v,feat_a,feat_s,feat_d).T
                    volumeT = volumeT / self.contra_temp
                else:
                    volumeT = volume_computation4(feat_b_all,feat_v,feat_a,feat_s).T
                    volumeT = volumeT / self.contra_temp
            else:
                volumeT = volume_computation3(feat_b_all,feat_v,feat_a).T
                volumeT = volumeT / self.contra_temp
            rank = dist.get_rank()
            bs = feat_b.size(0)
            ...
            # loss_area.append(torch.tensor(0))

            #  MWB  loss computation
            potential_loss = 0
            mse_bary_loss = 0
            if "raw_subtitles" in batch.keys():
                if "depth_pixels" in batch.keys():
                    feat_other = [feat_t_all, feat_v_all, feat_a_all, feat_s_all, feat_d_all]
                    for i in range (self.config.modality_num):
                        mse_bary_loss += self.Lambda[i] * torch.mean((feat_b_all - feat_other[i])**2 ) * 0.5
                else:
                    for i in range (self.config.modality_num):
                        feat_other = [feat_t_all,feat_v_all, feat_a_all, feat_s_all]
                        mse_bary_loss += self.Lambda[i] * torch.mean((feat_b_all - feat_other[i])**2 ) * 0.5
            else:
                for i in range (self.config.modality_num):
                    feat_other = [feat_t_all,feat_v_all, feat_a_all]
                    mse_bary_loss += self.Lambda[i] * torch.mean((feat_b_all - feat_other[i])**2 ) * 0.5

            feat_b_all_reshape = feat_b_all.view(1,self.config.contra_dim,-1)
            for i in range(self.config.modality_num):
                potential_loss += self.Lambda[i] * self.potentials(feat_b_all_reshape, i).mean()
            loss = mse_bary_loss -  potential_loss
            loss_pot_main.append(loss)


            ## 2nd forward potential loss
            potential_loss = 0
            for i in range(self.config.modality_num):
                potential_constraint = 0
                feat = feat_b_all_reshape.detach()  
                potential_loss += self.Lambda[i] * self.potentials(feat, i).mean()

                for j in range(self.config.modality_num):
                    potential_constraint += self.Lambda[j] * self.potentials(feat, j).mean()
                
                potential_loss += 10 * (potential_constraint ** 2) / self.config.modality_num

            loss =  potential_loss
            loss_pot.append(loss)
            ...
            for task in subtasks:
                ...
                # sim_cond2t = torch.matmul(feat_cond, feat_b_all.permute(1,0))
                # sim_cond2t = sim_cond2t / self.contra_temp
                # sim_t2cond = torch.matmul(feat_b, feat_cond_all.permute(1,0))
            ...
            loss_pot_main = sum(loss_pot_main)/len(loss_pot_main)
            loss_dict['loss_pot_main']= loss_pot_main
            loss_pot = sum(loss_pot)/len(loss_pot)
            loss_dict['loss_pot']= loss_pot
