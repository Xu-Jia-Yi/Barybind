...
def evaluate_ret(model, tasks, val_loader, global_step):
    ...
    for i, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        feat_b.append(evaluation_dict['feat_b'])
    ...
    if len(feat_s)>0:
        if len(feat_d)>0:
            area = volume_computation5(feat_b,feat_v,feat_a,feat_s,feat_d)
        else:
            area = volume_computation4(feat_b,feat_v,feat_a,feat_s) #(feat_t,feat_v,feat_a)
    else:
        area = volume_computation3(feat_b,feat_v,feat_a)
        
    min_values_volume = torch.min(area, 1).values
    mean_values_volume = torch.mean(min_values_volume)
    val_log[f"volume_value"] = {"value": mean_values_volume.item()}
    ...
    for task in subtasks:
        ...
        if task=='tv':
            score_matrix_t_cond = torch.matmul(feat_b, feat_v.permute(1,0))
        elif task=='ta':
            score_matrix_t_cond = torch.matmul(feat_b, feat_a.permute(1,0))
    ...
    #### compute itm_score
    for task in subtasks:
        if  task == "tvas" or task == "tva" or task ==  "tv ":
            continue
        if task!="tvas" and task!="tva" and task!="tv":
            store_dict[f'condition_feats_{task}'] = torch.cat(store_dict[f'condition_feats_{task}'],dim=0)
        itm_rerank_num = model.config.itm_rerank_num
    ...
    if dist.get_rank() == 0:
        ...
        topk = 5
        sim_matrix = torch.matmul(feat_b, feat_v.T).cpu().numpy()  # 计算相似度矩阵
        results = []
        for i, txt_id in enumerate(ids_txt):
            scores = sim_matrix[i]
            topk_idx = scores.argsort()[::-1][:topk]
            topk_video_ids = [ids[j] for j in topk_idx]
            topk_scores = [float(scores[j]) for j in topk_idx]
            results.append({
                "text_id": txt_id,
                "topk_video_ids": topk_video_ids,
                "topk_scores": topk_scores,
            })

        save_path = f"./results/retrieval_results_step_{global_step}.json"
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"Saved retrieval results JSON to {save_path}")
