...
def evaluate_ret(model, tasks, val_loader, global_step):
    ...
    for i, batch in tqdm(enumerate(val_loader),total=len(val_loader)):
        ...
        # 提取 caption 文本
        captions += batch['raw_captions']  # list of str, same length as batch size

        if 'ids_txt' in batch:
            ...
            ids_txt  += batch.ids_txt
            # 假设 caption_txt 也存在，或者你根据 ids_txt 映射回 caption
            captions_txt += batch['raw_captions']  # <-- 你需要构建这个
        else:    
            ids_txt  += batch.ids
            captions_txt += batch['raw_captions']  # 兜底方式，按当前 batch caption 填
    ...
    captions = [j for i in all_gather_list(captions) for j in i]
    captions_txt = [j for i in all_gather_list(captions_txt) for j in i]
    ...
    for task in subtasks:
        ...
        score_matrix_t_cond = torch.matmul(feat_b, store_dict[f'feat_cond_{task}'].permute(1,0))
        ...
        log = compute_metric_ret(score_matrix_t_cond, captions, captions_txt, direction='forward') ## caption相同视为正确
    ...
    for task in subtasks:
        ...
        log = compute_metric_ret(score_matrix, captions, captions_txt, direction='forward') ## caption
        log = {k.replace('forward','video'): v for k,v in log.items()}
...
def compute_metric_ret(score_matrix, ids, ids_txt, direction='forward'):
    ...
    print("示例 query:", ids_txt[:3])
    print("示例 candidate:", ids[:3])
    ...
    if direction == 'forward': ### text-to-vision retrieval
        ...
        for i in range(len(ids_txt)):
            gt_text = ids_txt[i]
            # 找出所有相同的候选（允许重复）
            gt_indices = [j for j, cand in enumerate(ids) if cand == gt_text]

            rank_i = min([indice_matrix[i].index(j) for j in gt_indices])
            rank.append(rank_i)
