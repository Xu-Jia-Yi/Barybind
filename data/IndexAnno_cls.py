        ...
        if self.vision_mapper:
            if self.vision_mapper.vision_format == 'video_feats':
                ...
                if vision_pixels is None:
                    if self.training: 
                        resample_idx = random.choice(self.idx)
                        LOGGER.info(f'[vision] current idx {id_} from {self.dataset_name} returns wrong image/video, use {resample_idx} instead.')
                        return self.__getitem__(resample_idx)
                    else:
                        LOGGER.warning(f'[vision] current idx {id_} from {self.dataset_name} returns missing video/image. Skipping.')
                        return None
...
def annoindexedcollate(inputs):
    ...
    if len(inputs) == 0:
        return None  # 如果全部都为 None，则返回 None，外层逻辑需判断并跳过
